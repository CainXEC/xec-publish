-- The single notifications store: one row per event delivered to a recipient
-- account. Keyed by public.accounts(id) on BOTH sides, because it represents
-- everyone — reader-only accounts (most feed posters have no author_id) and
-- authors alike. The old author-keyed `notifications` table is retired; the
-- article "follow author" now records here too (see app/api/follows/route.js).
--
-- Event types, all fired best-effort at the moment the action is recorded (see
-- lib/feedNotifications.js and the confirm/react/follow/unlock/comment paths):
--   reply / quote / like / repost  -> post_txid = the feed post the recipient owns
--   follow                          -> post_txid NULL (feed or article follow)
--   offer                           -> post_txid = the handle's token id
--   unlock / comment                -> post_txid = the ARTICLE's post id
-- (the offer + unlock/comment types are added by the sibling migrations
--  handle_offers.sql and feed_notifications_article_types.sql).
--
