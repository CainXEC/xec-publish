 ...
 * `since` matches unlock/earnings window for earned/unlocks sorts; use `null` for newest (all-time unlocks).
 */
async function fetchPinnedHomePost(supabase, { since, earnedSort, followedAuthorIds }) {
  ...
  const [unlockRes, commentRes] = await Promise.all([
    supabase.rpc('get_unlock_counts', { post_ids: [postId], since }),
...
export async function getHomepagePosts({
  ...
  if (allMeta.length === 0) {
    return { pinnedPost, posts: [], hasNextPage: false }
  }
  ...
  return { pinnedPost, posts, hasNextPage }
}
