...
export async function resolveCommenter(request, postId, supabase) {
  ...
  if (acct) {
    ...
    // Logged-in reader (admins included) — allowed only if this account has an
    // unlock on record, matched over EVERY address it has proven (login wallet,
    // still-linked old wallet, or its Pocket), not just the session address.
    // An unlock paid from the Pocket must still let that account comment.
    if (acct.accountId) {
