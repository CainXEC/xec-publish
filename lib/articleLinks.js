...
// Like ARTICLE_LINK_RE but consumes the whole URL token (slug + any query/hash)
// so it can be spliced out of displayed post text.
const ARTICLE_LINK_STRIP_RE = /(?:https?:\/\/[^\s/]+)?\/posts\/[^\s]*/i
...
/**
 * Remove the first on-site article URL from `content` for display — the feed
 * renders a preview card that is itself the link, so the raw URL is redundant.
 * Collapses the whitespace the removed token leaves behind and trims the ends.
 */
export function stripArticleLink(content) {
  const text = typeof content === 'string' ? content : ''
  if (!text) return text
  return text
    .replace(ARTICLE_LINK_STRIP_RE, '')
    .replace(/[^\S\n]+\n/g, '\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim()
}
