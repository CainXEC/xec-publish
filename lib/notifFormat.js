...
export function targetHref(n) {
  ...
  if (n.type === 'follow' || n.type === 'tip') {
    // Strip a leading "@" (handle) or "ecash:" (raw-address identity) so the link
    // is the canonical /@handle or /@<bare-address>, never /@ecash:<addr>.
    const id = String(n.actor_identity ?? '').replace(/^@/, '').replace(/^ecash:/i, '').trim()
  ...
  // A comment (or reply-to-comment) jumps straight to that comment — action_txid
  // is its own on-chain txid, which ArticleComments renders as a DOM id
  // (#comment-<txid>). An old row from before action_txid existed falls back to
  // the comments section itself.
  if (n.type === 'comment') {
    if (!n.articleHref) return '#'
    return n.action_txid ? `${n.articleHref}#comment-${n.action_txid}` : `${n.articleHref}#comments`
  }
  // A reply opens straight at the reply itself, not the post it replied to —
  // action_txid is the reply's own feed txid. A plain feed reply's permalink
  // page focuses on it directly; a forum reply's page resolves up to the forum
  // root, where FeedThreadClient/ForumComments scroll it into view via the same
  // #post-<txid> anchor. Falls back to the target post for an old row recorded
  // before action_txid existed.
  if (n.type === 'reply') {
    const txid = n.action_txid || n.post_txid
    return txid ? `/feed/${txid}#post-${txid}` : '#'
  }
  // The runner's forum-engagement earning. A REPLY in their forum carries the
  // reply's OWN txid in action_txid; a REACTION carries the reacted-to post in
  // post_txid. Either way jump straight to that specific post via the #post-<txid>
  // anchor — same as a reply notification — instead of dropping the runner at the
  // forum root (post_txid alone, no anchor), which is what fell through to the
  // default below and made these land on the whole thread, not the reply.
  if (n.type === 'forum_fee') {
    const txid = n.action_txid || n.post_txid
    return txid ? `/feed/${txid}#post-${txid}` : '#'
  }
