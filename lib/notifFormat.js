...
export function notifText(n) {
  ...
  if (n.type === 'like' || n.type === 'repost') return VERB[n.type]
  ...
  // A mention in an article names it ("mentioned you in <Title>"); a feed mention
  // shows the post text inline below, so the verb stays bare.
  if (n.type === 'mention') {
...
export function targetHref(n) {
  ...
  // A mention: the article it's in (article-linked) or the feed post (post_txid is
  // the post's own 64-hex txid). articleHref is only set for article mentions.
  if (n.type === 'mention') {
    if (n.articleHref) return n.articleHref
