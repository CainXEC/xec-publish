...
export const getPublishedPostBySlug = cache(async (rawSlug, legacy = false) => {
  ...
  const [unlockRes, commentRes, earnedRes, acctRes] = await Promise.all([
    ...
    // graph (feed_follows). Articles are author_id-keyed but follows are
    // account-keyed, so the follow button resolves author -> account here.
    supabase.from('accounts').select('id').eq('author_id', postRow.author_id).maybeSingle(),
  ])
  ...
  return {
    ...
    authorAccountId: acctRes?.data?.id ?? null,
