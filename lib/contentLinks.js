...
//      4. an X (Twitter) URL     -> opens the tweet in a NEW TAB (outbound; the
//                                   ONE external host allowed, no embed)
//
...
// X (Twitter) is the ONE external host whose links are "live": a pasted X/Twitter
// URL becomes a clickable link that OPENS IN A NEW TAB (no embed — the tweet isn't
// pulled onto the page). Every other external URL stays inert text, per the
// on-site-links-only policy. This mirrors the YouTube media exception, but as a
// plain outbound link rather than an inline player.
...
/**
 * The normalized absolute https URL for a pasted X/Twitter link, else null (not an
 * X host, non-http, or unparseable). Callers use it as the `href` of an outbound
 * (new-tab) link — it is always an absolute http(s) URL, never javascript:/data:.
 */
export function xUrlHref(rawUrl) {
  let u
  try {
    u = new URL(String(rawUrl ?? ''))
  } catch {
  ...
  if (u.protocol !== 'http:' && u.protocol !== 'https:') return null
  return X_HOSTS.has(u.hostname.toLowerCase()) ? u.toString() : null
}
...
export function tokenizeUrls(text) {
  ...
  while ((m = re.exec(src))) {
    ...
    if (href) {
      ...
      // On-site link? handled above. Otherwise the ONE live external kind is an
      // X/Twitter link (opens in a new tab); everything else stays inert text.
      const xhref = xUrlHref(url)
      tokens.push(xhref ? { type: 'xlink', href: xhref, value: url } : { type: 'text', value: url })
    }
