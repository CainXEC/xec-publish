...
//      4. an X (Twitter) URL or an e.cash URL (e.cash + any *.e.cash subdomain,
//         e.g. explorer.e.cash) -> opens in a NEW TAB (outbound; the allowed
//         external hosts, no embed)
//
...
// The external hosts whose links are "live": a pasted URL to one of these becomes
// a clickable link that OPENS IN A NEW TAB (no embed — the page isn't pulled in).
// Every OTHER external URL stays inert text, per the on-site-links-only policy.
//   • X (Twitter): the tweet opens in a new tab (mirrors the YouTube exception,
//     but as a plain outbound link rather than an inline player).
//   • e.cash: the chain's own sites — e.cash itself and ANY subdomain
//     (explorer.e.cash, avalanche.e.cash, …) — so on-chain proofs (a tx / address
//     / block on the explorer) can link out, fitting an eCash-native platform.
...
/** True for e.cash and any of its subdomains (case-insensitive). The dotted
 *  suffix test is deliberate: "evile.cash" and "e.cash.evil.com" do NOT match. */
function isEcashHost(host) {
  const h = String(host ?? '').toLowerCase()
  return h === 'e.cash' || h.endsWith('.e.cash')
}

/**
 * The normalized absolute https URL for a pasted link to an ALLOWED external host
 * (X/Twitter or any e.cash host), else null (other host, non-http, unparseable).
 * Callers use it as the `href` of an outbound (new-tab) link — it is always an
 * absolute http(s) URL, never javascript:/data:.
 */
export function externalUrlHref(rawUrl) {
  ...
  return X_HOSTS.has(host) || isEcashHost(host) ? u.toString() : null
}
...
export function tokenizeUrls(text) {
  ...
  while ((m = re.exec(src))) {
    ...
    if (href) {
      ...
      // On-site link? handled above. Otherwise the live external kinds are an
      // X/Twitter link or an e.cash link (each opens in a new tab); everything
      // else stays inert text.
      const exthref = externalUrlHref(url)
