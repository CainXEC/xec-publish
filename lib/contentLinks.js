...
/**
 * A byline identity → its on-site profile path, or null if it isn't linkable.
 * "@handle" → /@handle; a raw 42-char eCash address (with or without the
 * "ecash:" prefix) → /@<address> (the profile route resolves either, via
 * resolveProfileByIdentifier). Placeholder bylines (Anonymous, a [deleted]
 * tombstone) and partial addresses return null. The single home for a rule
 * that was inline-copied in ArticleComments.js and app/api/activity/route.ts.
 */
export function profileHrefForIdentity(identity) {
...
// Legacy root permalink (see app/[slug]/page.js): imported archive posts with
// legacy=true keep their ORIGINAL URL at bare "/<slug>" (e.g. /26, /1, /00) —
// one path segment, no further nesting. Without this, a link to one of those
// (which the editor happily auto-links while composing) fails re-validation
// here and silently loses its href at read time (see sanitizePostBodyHtml),
// rendering as inert text with no visible reason why.
const LEGACY_SLUG_RE = /^\/[^\s/?#]+(?:[?#][^\s]*)?$/
...
export function powInternalHref(rawHref) {
  ...
  return LIVE_PATH_RE.test(path) || LEGACY_SLUG_RE.test(path) ? path : null
}
...
// A cheap "could this possibly contain a link" pre-check, built from the SAME
// pieces tokenizeContent itself uses (a mention char, a scheme, or one of the
// bare-linkable domains) — never drift out of sync with what the real
// tokenizer detects. A caller that wants to skip full tokenization for most
// text (e.g. the article publish-time transform, walking every text node)
// should filter with this, not a hand-rolled regex — a narrower one (e.g.
// requiring "https://") would silently miss a bare "proofofwriting.com/…" link.
const MAY_CONTAIN_LINK_RE = new RegExp(`@[A-Za-z0-9_]|https?:\\/\\/|${BARE_DOMAIN_ALT}`, 'i')

/** True if `text` might contain a mention or URL tokenizeContent would turn
 *  into a link — a cheap pre-filter for callers that want to skip full
 *  tokenization of text that plainly has neither. */
export function textMayContainLink(text) {
  return MAY_CONTAIN_LINK_RE.test(typeof text === 'string' ? text : '')
}
