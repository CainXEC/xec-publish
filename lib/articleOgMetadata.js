...
// v5: render at 1200x630 (was 2400x1260) — the 2x card took ~4s and overran
// social crawlers' timeout, so X dropped it.
// v6: cache-bust the image URL. X caches the card IMAGE by its URL, separate from
//     the page URL, so a page-level cache-buster (?s=1) can't refresh it. Some
//     cards were stuck showing text with a blank image — likely X cached a
//     cold-start miss on the v5 image. The template is unchanged; bumping v just
//     hands X a fresh image URL to fetch (now ~1s, well under its timeout).
