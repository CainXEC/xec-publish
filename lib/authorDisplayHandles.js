 ...
 *
 * Returns `{ handle, color }` per author: `color` is the account's chosen handle
 * color (one of the approved theme swatches) or null for the theme default.
 *
 ...
 * @returns {Promise<Record<string, { handle: string, color: string|null }>>}
 */
export async function displayHandlesByAuthorId(authorIds, client) {
  ...
  const { data } = await supabase
    .from('accounts')
    .select('author_id, display_handle, handle_color')
    .in('author_id', ids)
  ...
  for (const row of data ?? []) {
    ...
    if (row?.author_id && row.display_handle && !map[row.author_id]) {
      map[row.author_id] = { handle: row.display_handle, color: row.handle_color ?? null }
    }
  }
  return map
}
 ...
 *
 * Returns `{ handle, color }` per account: `color` is the account's chosen handle
 * color (one of the approved theme swatches) or null for the theme default.
 *
...
export async function displayHandlesByAccountId(accountIds, client) {
  ...
  for (const row of data ?? []) {
    if (row?.id && row.display_handle && !map[row.id]) {
      map[row.id] = { handle: row.display_handle, color: row.handle_color ?? null }
    }
  }
  return map
}
