...
// span's mints are fetched separately and woven back in as individual compact
// rows — one @proofofwriting announcement per handle minted.
// Newest-first fetch cap for a span's mint rows (bounds how many mint
// announcements a single gap between real posts can hold).
...
/**
 * Every span mint rides as its own compact row — one @proofofwriting
 * announcement per handle minted ("@handle minted · price"), no digest
 * collapsing. Bounded per span by MINT_SPAN_FETCH_LIMIT.
 */
function buildMintEntries(mints) {
  return { inlineMints: mints ?? [], digest: null }
}
...
async function finalizeWindow(
  ...
  const { inlineMints, digest } = buildMintEntries(spanMintsResolved?.mints)
