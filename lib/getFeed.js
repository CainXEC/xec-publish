...
const FEED_POST_COLUMNS =
  'id, txid, action, parent_txid, quoted_txid, content, content_hash, author_account_id, author_identity, payer_address, payout_address, amount_sats, created_at, deleted_at, reply_count, like_count, repost_count, quote_count'
...
// The Following feed additionally surfaces the replies of people you follow,
// each shown with a "replying to @X" context line (see attachParentPreview).
const FOLLOWING_ACTIONS = [FEED_ACTION.POST, FEED_ACTION.QUOTE, FEED_ACTION.REPLY]
...
async function attachLiveIdentity(supabase, posts) {
  ...
  return posts.map((p) => ({
    ...
    parent: p.parent ? { ...p.parent, displayIdentity: identityFor(p.parent) } : p.parent,
  }))
}
...
async function decoratePosts(
  supabase,
  rows,
  { viewerAddress = '', viewerAccountId = null, withParents = false } = {},
) {
  ...
  const withParent = withParents
    ? await attachParentPreview(supabase, withQuoted)
    : withQuoted
  const withFollow = await attachFollowState(supabase, withParent, viewerAccountId)
...
async function finalizeWindow(
  ...
  rows,
  pageSizeNum,
  { viewerAddress, viewerAccountId, withParents = false },
) {
  const rowsList = rows ?? []
