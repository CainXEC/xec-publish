...
async function attachLiveIdentity(supabase, posts) {
  ...
  // The poster's handle-card art, for the feed byline avatar — only when they
  // currently show a handle (an address byline has no card to show). Falls back
  // to the deterministic render route when the hosted PNG isn't set yet, same as
  // every other card-art surface (HandleCardImage.tsx).
  const avatarFor = (row) => {
    const entry = row ? entryFor(row) : null
    if (!entry?.handle) return null
    return entry.cardImageUrl || (entry.tokenId ? `/api/handle-card/${entry.tokenId}` : null)
  }
  ...
  return posts.map((p) => ({
    ...
    // Main post author only for now (not quoted/parent/topReply/repost —
    // those bylines stay text-only until this extends further).
    displayAvatarUrl: avatarFor(p),
