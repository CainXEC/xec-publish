...
async function decoratePosts(
  ...
  const withLinked = await attachLinkedPosts(supabase, withCards)
  ...
  return attachIdentity ? attachLiveIdentity(supabase, withLinked) : withLinked
}
...
async function finalizeWindow(
  ...
  rows,
  pageSizeNum,
  { viewerAddress, viewerAccountId, withParents = false, attachIdentity = true },
) {
  const rowsList = rows ?? []
  const full = rowsList.length === pageSizeNum
  const nextCursor = full ? encodeCursor(rowsList[rowsList.length - 1]) : null

  const signals = await fetchEngagementSignals(supabase, rowsList)
  const ranked = rankFeedCandidates(rowsList, signals)
...
export async function getFeedPage({ cursor = null, pageSize = 25, viewerAddress = '', viewerAccountId = null, attachIdentity = true } = {}) {
  ...
  return finalizeWindow(supabase, rows, pageSizeNum, { viewerAddress, viewerAccountId, attachIdentity })
}
