...
// How many posts a "For You" page fetches, ranks, and shows. This is also the
// window the per-viewer follow-nudge reorders within (a bigger window lets the
// nudge lift a followed author from deeper in the page), so it's the single dial
// for both page weight and nudge reach.
export const FORYOU_PAGE_SIZE = 40
...
const FEED_POST_COLUMNS =
  'id, txid, action, parent_txid, quoted_txid, content, content_hash, title, author_account_id, author_identity, payer_address, payout_address, amount_sats, forum_id, created_at, deleted_at, reply_count, like_count, repost_count, quote_count, reaction_counts, card_kind, image_url, card_meta'
...
function toClientPost(row) {
  ...
  // Forum posts with a title store `title \n\n body` in content; split so the
  // client gets a clean title + body. `title` NULL (feed posts, replies, and the
  // pre-title forum posts) leaves content untouched.
  const hasTitle = row.title != null
  const body = hasTitle ? splitForumContent(row.content).body : row.content
  return {
    ...row,
    title: deleted ? null : (hasTitle ? row.title : null),
...
async function attachQuoted(supabase, posts, blockedIds = null) {
  ...
  await attachForumContextToPreviews(supabase, Object.values(byTxid))
 ...
 */
/**
 * Enrich quoted/linked-post PREVIEW objects with their forum context, so the
 * embed can label a shared forum post as "in /f/<slug>" (or a comment). Sets
 * `forumSlug` + `isForumComment` (action 2) on each preview that lives in a
 * forum; a non-forum preview is untouched. One batched forums lookup.
 */
async function attachForumContextToPreviews(supabase, previews) {
  const forumIds = [...new Set(previews.map((p) => p?.forum_id).filter(Boolean))]
  if (forumIds.length === 0) return
  const { data } = await supabase.from('forums').select('id, slug').in('id', forumIds)
  const slugById = new Map((data ?? []).map((f) => [f.id, f.slug]))
  for (const p of previews) {
    if (p?.forum_id) {
      p.forumSlug = slugById.get(p.forum_id) ?? null
      p.isForumComment = p.action === 2
    }
  }
}
 ...
 */
/**
 * The subset of THIS window's author account-ids that `viewer` follows — the
 * input to the follow nudge. Scoped to the page's authors (a tiny IN list), so
 * it's one cheap query per viewer per page, layered on the shared cached window
 * (never inside the viewer-neutral cache).
 */
async function followedAuthorsInWindow(supabase, viewer, posts) {
  if (!viewer) return new Set()
  const authorIds = [
    ...new Set(posts.map((p) => p.author_account_id).filter((id) => id && id !== viewer)),
  ]
  ...
  return new Set((data ?? []).map((r) => r.followee_account_id))
}
...
/**
 * Attach `authorFollowerCount`: how many accounts follow each post's author
 * (feed_follows.followee_account_id). Viewer-neutral, so it lives in the shared/
 * cached window. Address-only posters (no account) get null (no follower concept).
 * One head:true COUNT per unique author (no rows transferred), run in parallel.
 */
async function attachFollowerCounts(supabase, posts) {
  const ids = [...new Set((posts ?? []).map((p) => p.author_account_id).filter(Boolean))]
  ...
  await Promise.all(
    ids.map(async (id) => {
      const { count } = await supabase
        ...
        .eq('followee_account_id', id)
      counts.set(id, count ?? 0)
    }),
  )
  return posts.map((p) => ({
    ...p,
    authorFollowerCount: p.author_account_id ? counts.get(p.author_account_id) ?? 0 : null,
  }))
}
...
/**
 * When a post's text links to an on-site forum (/f/<slug>), attach a shallow
 * preview of that forum as `post.forumCard` so the feed can render a link card
 * (mirrors attachArticleCards). Resolved by the forum's confusable-folded
 * skeleton, so /f/Bitcoin and /f/bitcoin match the same forum. Deleted posts
 * (content withheld) are skipped.
 */
async function attachForumCards(supabase, posts) {
  const slugByPost = new Map()
  for (const p of posts) {
    if (p.deleted) continue
    const slug = extractForumSlug(p.content)
    if (slug) slugByPost.set(p, slug)
  }
  if (slugByPost.size === 0) return posts.map((p) => ({ ...p, forumCard: null }))

  const skels = [...new Set([...slugByPost.values()].map((s) => skeleton(s)))]
  const { data: rows } = await supabase
    .from('forums')
    .select('slug, slug_skeleton, title, description, post_count, runner_account_id')
    .in('slug_skeleton', skels)
  ...
  return posts.map((p) => {
    const slug = slugByPost.get(p)
    return { ...p, forumCard: slug ? bySkel[skeleton(slug)] ?? null : null }
  })
}
...
async function decoratePosts(
  ...
  const [withReactions, withQuoted, withParent, withFollow, withFollowerCounts, withCards, withLinked, withTop, withForum] =
    await Promise.all([
      ...
      attachFollowerCounts(supabase, shaped),
      ...
      attachForumCards(supabase, shaped),
    ])
 ...
 * supporters per post, with self/linked-wallet + house/AI reactions already
 ...
 * a missing/failed signal yields an empty Map and the ranker degrades gracefully
 * to recency + conversation.
 */
 ...
 *
 * Mint announcements (card_kind = 'handle_mint') are excluded from the feed
 * ENTIRELY — the window is real writing only, and mints are no longer woven in
 * (they cluttered the feed, and a stale one could dominate the top). They still
 * live full-size on the @proofofwriting profile, the thread page, and inside quote
 * embeds. (The digest/weave helpers are dormant, kept for easy re-enable.)
...
export async function getFeedPage({ cursor = null, pageSize = 25, viewerAddress = '', viewerAccountId = null, attachIdentity = true } = {}) {
  ...
  const FETCH_SIZE = pageSizeNum * 2 + 5
  ...
  postsQuery = applyCursor(postsQuery, decoded).limit(FETCH_SIZE)

  const [{ data: rows, error }, repostEntries] = await Promise.all([
    postsQuery,
    fetchRepostEntries(supabase, decoded, FETCH_SIZE),
  ])
  if (error) throw new Error(error.message)

  const { window, nextCursor } = buildAccountTimelineWindow(rows, repostEntries, pageSizeNum)

  const posts = await decoratePosts(supabase, window, {
    viewerAddress,
    viewerAccountId,
    attachIdentity,
    withTopReply: true,
...
// How many posts the "Top" tab ranks + shows in one shot (no load-more — a
// forum's most-engaged posts are a leaderboard, not an infinite scroll).
const FORUM_TOP_LIMIT = 50

/**
 * A page of ONE forum's posts (top-level) — the forum page's feed. Mirrors
 * getFeedPage but scoped to a single forum_id; no mint digest (mints belong to
 * the global Feed). Two orderings, matching the forum's tabs:
 *   - sort='new' (default): strictly newest-first, keyset paginated (rank=false
 *     so it's a true chronological "New", not engagement-reordered).
 *   - sort='top': the forum's leaderboard — most comments, then most reactions,
 *     then newest — in a single capped page (nextCursor always null).
 * Both use the denormalized counts already on the row, so "Top" needs no extra
 * query. For a very large forum, "Top" ranks the whole table by reply_count (an
 * index-friendly single sort); deep historical ranking is a later refinement.
 */
export async function getForumFeedPage({ forumId, cursor = null, pageSize = 25, sort = 'new', viewerAddress = '', viewerAccountId = null } = {}) {
  const supabase = adminDb()
  const base = supabase
    .from('feed_posts')
    .select(FEED_POST_COLUMNS)
  ...
  if (sort === 'top') {
    const { data: rows, error } = await base
      ...
      .order('created_at', { ascending: false })
      .order('id', { ascending: false })
      .limit(FORUM_TOP_LIMIT)
    if (error) throw new Error(error.message)
    const page = await finalizeWindow(supabase, rows, FORUM_TOP_LIMIT, {
      viewerAddress,
      viewerAccountId,
      attachIdentity: true,
      spanMints: Promise.resolve([]),
      rank: false, // keep the DB leaderboard order (don't re-blend by recency)
    })
    return { ...page, nextCursor: null } // Top is a single capped page
  }
  ...
  return finalizeWindow(supabase, rows, pageSizeNum, {
    ...
    attachIdentity: true,
    spanMints: Promise.resolve([]),
    rank: false, // "New" is a true chronological timeline, not an engagement re-rank
  })
}
 ...
 *
 * `viewerAccountId` may be a plain value OR an in-flight Promise (awaiting a
 * non-promise just resolves immediately, so either works). Every caller was
 * resolving its own session lookup FIRST and only then calling this — one full
 * network round trip blocking a second one that doesn't actually depend on it
 * (the cached window fetch below needs no viewer identity at all). Accepting a
 * promise lets a caller pass its still-pending getAuthedAccount() call directly,
 * so the session lookup and the cache lookup run concurrently instead of
 * serially; this function is the one place that awaits it, right before the
 * identity is actually needed for the blocked-accounts filter.
 */
export async function getCachedForYouPage(cursor = null, pageSize = 25, viewerAccountId = null) {
  ...
  const pagePromise = unstable_cache(
    () =>
      getFeedPage({
  ...
  // Runs alongside pagePromise instead of after it — see the doc comment above.
  const [page, resolvedViewerAccountId] = await Promise.all([pagePromise, viewerAccountId])
  const supabase = adminDb()
  let posts = page.posts
  if (resolvedViewerAccountId) {
    // Blocks REMOVE posts; the follow nudge only REORDERS. While the feed is
    // reverse-chronological (RANK_FORYOU_FEED=false) there's nothing to reorder,
    // so only the block filter runs — and we skip the follow-set query entirely.
    // nextCursor is time-based (finalizeWindow), so any reordering, when enabled,
    // never affects pagination.
    const [blocked, followed] = await Promise.all([
      blockedAccountIds(supabase, resolvedViewerAccountId),
      RANK_FORYOU_FEED
        ? followedAuthorsInWindow(supabase, resolvedViewerAccountId, posts)
    ...
    if (RANK_FORYOU_FEED) posts = applyFollowNudge(posts, followed)
  }
 ...
 */
/**
 * Fetch repost events and hydrate the original posts so they can be merged into
 * a timeline. Pass actorAccountId to restrict to one account (profile feed);
 * omit it for the global feed (all reposters). Returns entries shaped like
 * feed_posts rows plus:
 *   _surfaceTs — the repost's created_at (surfacing time for sort/cursor)
 *   _surfaceId — the repost event's id (tiebreaker within feed_events)
 *   repostedBy — { accountId, identity } for the "Reposted by @X" byline
 */
async function fetchRepostEntries(supabase, decoded, fetchSize, actorAccountId = null) {
  let repostsQuery = supabase
    .from('feed_events')
    .select('id, target_txid, created_at, actor_account_id, actor_identity')
    .eq('action', FEED_ACTION.REPOST)
    .order('created_at', { ascending: false })
    .order('id', { ascending: false })
  if (actorAccountId) repostsQuery = repostsQuery.eq('actor_account_id', actorAccountId)
  if (decoded) {
    repostsQuery = repostsQuery.or(
      `created_at.lt.${decoded.createdAt},and(created_at.eq.${decoded.createdAt},id.lt.${decoded.id})`,
    )
  }
  const { data: events } = await repostsQuery.limit(fetchSize)
  if (!events?.length) return []

  const txids = events.map((e) => e.target_txid).filter(Boolean)
  const { data: originals } = await supabase
    .from('feed_posts')
    .select(FEED_POST_COLUMNS)
    .in('txid', txids)
    .is('deleted_at', null)
  const byTxid = new Map((originals ?? []).map((p) => [p.txid, p]))

  return events.flatMap((e) => {
    const orig = byTxid.get(e.target_txid)
    if (!orig) return []
    return [{ ...orig, _surfaceTs: e.created_at, _surfaceId: e.id, repostedBy: { accountId: e.actor_account_id, identity: e.actor_identity } }]
  })
}

/**
 * Merge own posts (tagged with _surfaceTs = created_at) and repost entries,
 * dedup by txid (most-recent surfacing wins), sort by surfacing time, take
 * pageSizeNum. Returns { window, nextCursor }.
 */
function buildAccountTimelineWindow(ownRows, repostEntries, pageSizeNum) {
  const ownTagged = (ownRows ?? []).map((r) => ({ ...r, _surfaceTs: r.created_at, _surfaceId: r.id, repostedBy: r.repostedBy ?? null }))
  const all = [...ownTagged, ...repostEntries].sort((a, b) => {
    const dt = new Date(b._surfaceTs) - new Date(a._surfaceTs)
    if (dt !== 0) return dt
    return b._surfaceId > a._surfaceId ? 1 : -1
  })
  const seen = new Set()
  const merged = []
  for (const e of all) {
  ...
  const window = merged.slice(0, pageSizeNum)
  const last = window[window.length - 1]
  const nextCursor =
    merged.length > pageSizeNum && last
      ? encodeCursor({ created_at: last._surfaceTs, id: last._surfaceId })
      : null
...
export async function getAccountFeedPage({ accountId, cursor = null, pageSize = 25, viewerAddress = '', viewerAccountId = null } = {}) {
  ...
  let postsQuery = supabase
    ...
    .order('created_at', { ascending: false })
    .order('id', { ascending: false })
  if (pinnedTxid) postsQuery = postsQuery.neq('txid', pinnedTxid)
  postsQuery = applyCursor(postsQuery, decoded).limit(FETCH_SIZE)

  const [{ data: rows, error }, repostEntries] = await Promise.all([
    postsQuery,
    fetchRepostEntries(supabase, decoded, FETCH_SIZE, account),
  ])
  if (error) throw new Error(error.message)

  const { window, nextCursor } = buildAccountTimelineWindow(rows, repostEntries, pageSizeNum)

  const posts = await decoratePosts(supabase, window, { viewerAddress, viewerAccountId, blockedIds: blocked })
  const page = { posts, nextCursor }
  return cursor
    ? page
...
async function accountFeedPageNeutral(supabase, accountId, pageSizeNum) {
  ...
  const [{ data: rows, error }, repostEntries] = await Promise.all([
    postsQuery.limit(FETCH_SIZE),
    fetchRepostEntries(supabase, null, FETCH_SIZE, accountId),
  ])
  if (error) throw new Error(error.message)

  const { window, nextCursor } = buildAccountTimelineWindow(rows, repostEntries, pageSizeNum)

  const posts = await decoratePosts(supabase, window, { viewerAddress: '', viewerAccountId: null, attachIdentity: false })
  const page = { posts, nextCursor }
  return prependPinnedPost(supabase, accountId, pinnedTxid, page, { viewerAddress: '', viewerAccountId: null, attachIdentity: false })
}
...
// A post rarely has a huge number of quotes; show the newest this many without
// pagination (a "load more" can come later if a post ever blows past it).
const POST_QUOTES_LIMIT = 50

/**
 * The posts that QUOTE `txid` — so a thread can show "who quoted this". Newest
 * first, decorated like any feed post (each carries its own `quoted` embed of the
 * original, plus the viewer's reaction/follow state), so they render in FeedPost
 * unchanged. Blocked authors are dropped.
 * @returns {Promise<{ posts: object[] }>}
 */
export async function getPostQuotes(txid, { viewerAddress = '', viewerAccountId = null } = {}) {
  ...
  const { data: rows, error } = await supabase
    .from('feed_posts')
    .select(FEED_POST_COLUMNS)
    .eq('action', FEED_ACTION.QUOTE)
    .eq('quoted_txid', clean)
    .is('deleted_at', null)
    .order('created_at', { ascending: false })
    .order('id', { ascending: false })
    .limit(POST_QUOTES_LIMIT)
  if (error) throw new Error(error.message)

  const page = await finalizeWindow(supabase, rows, POST_QUOTES_LIMIT, {
    viewerAddress,
    viewerAccountId,
    blockedIds: blocked,
    rank: false, // chronological — newest quotes first
  })
...
/**
/**
 * Every reply BELOW `rootTxid` — the whole descendant tree, not just the direct
 * children — as a flat array (each row carries parent_txid so the client can
 * nest). BFS one level at a time (parent_txid IN the last level's txids), bounded
 * by depth + node caps so a pathological thread can't run away. Returns the same
 * `{ data, error }` shape as a Supabase query so getFeedThread treats both paths
 * identically. Powers the Reddit-style nested comment view on forum threads.
 */
async function fetchDescendantReplies(supabase, rootTxid, { maxDepth = 20, maxNodes = 800 } = {}) {
  const all = []
  const seen = new Set()
  let frontier = [rootTxid]
  let depth = 0
  while (frontier.length > 0 && depth < maxDepth && all.length < maxNodes) {
    const { data, error } = await supabase
      .from('feed_posts')
      .select(FEED_POST_COLUMNS)
      .eq('action', 2)
      .in('parent_txid', frontier)
      .is('deleted_at', null)
 ...
 * tombstone so the chain stays intact. `deep` returns the FULL descendant tree
 * as a flat `replies` array (forum threads render it nested).
 * @returns {Promise<{ post: object, ancestors: object[], replies: object[] } | null>}
 */
/**
 * The ROOT txid of a forum thread — the top-level post a comment descends from.
 * A shared forum link (or a live-rail item) can point at any comment deep in a
 * thread, but a forum always reads from the root DOWN (Reddit-style: the post,
 * then its whole nested comment tree), never as an X-style ancestor chain ending
 * on one focused comment. Both the standalone thread page and the reading pane
 * resolve to the root before rendering so a linked forum comment lands on the
 * same view the forum itself shows. Returns `txid` unchanged when it's already a
 * root (top-level post, no ancestors).
 */
export async function forumRootTxid(txid) {
  const clean = typeof txid === 'string' ? txid.trim().toLowerCase() : ''
  if (!clean) return clean
  const { data } = await adminDb().rpc('get_feed_ancestors', {
    p_txid: clean,
    p_max_depth: MAX_ANCESTORS,
  })
  // Rows are root-first (deepest ancestor = root); [0] is the top-level post.
  const rows = data ?? []
  return rows.length ? rows[0].txid : clean
}

// Shape a flat feed thread from the focused post's full descendant subtree:
//   • the focused post's DIRECT replies (one level, X-style), plus
//   • the focused author's SELF-REPLY continuation — walk down while the SAME
//     author keeps replying to their own last post (their tweetstorm), so the
//     whole author thread is visible even when opened from the top.
// Deeper sub-replies by OTHER people stay one level down (open that reply to see
// them). Returned in thread (DFS) order so each shown reply reads directly under
// what it answers. `subtree` is the flat descendant list from fetchDescendantReplies.
function feedThreadReplies(focusTxid, focusAuthor, subtree) {
  const childrenByParent = new Map()
  for (const r of subtree) {
    const kids = childrenByParent.get(r.parent_txid)
    if (kids) kids.push(r)
    else childrenByParent.set(r.parent_txid, [r])
  }
  const shown = new Set()
  // Direct replies to the focused post.
  for (const r of childrenByParent.get(focusTxid) ?? []) shown.add(r.txid)
  // Author self-reply continuation (the thread).
  if (focusAuthor) {
    let cur = focusTxid
    for (let i = 0; i < 1000; i++) {
      const selfKid = (childrenByParent.get(cur) ?? []).find(
        (k) => k.author_account_id === focusAuthor,
      )
      if (!selfKid) break
      shown.add(selfKid.txid)
      cur = selfKid.txid
    }
  }
  // DFS from the focused post, emitting only the shown set (chronological siblings).
  const out = []
  const visit = (txid) => {
    for (const k of childrenByParent.get(txid) ?? []) {
      if (shown.has(k.txid)) {
        out.push(k)
        visit(k.txid)
      }
...
export async function getFeedThread(txid, { viewerAddress = '', viewerAccountId = null, deep = false } = {}) {
  const clean = typeof txid === 'string' ? txid.trim().toLowerCase() : ''
  ...
  const [postRes, blocked, replyRes, ancestorRes] = await Promise.all([
    ...
    // Both views need the descendant tree now. A forum thread nests the WHOLE
    // tree (Reddit-style); a feed thread flattens the focused post's OWN self-reply
    // chain (a tweetstorm) into the thread so it reads fully from the top, then
    // trims the rest to one level (see feedThreadReplies below).
    fetchDescendantReplies(supabase, clean),
  ...
  const postAuthor = postRow.author_account_id ?? null
  // Forum threads render the WHOLE descendant tree (ForumComments nests it). The
  // flat feed thread instead shows the focused post's direct replies PLUS the
  // author's self-reply continuation (their tweetstorm), in thread order — so
  // opening the TOP of a thread reveals the whole thread, not just the next post.
  // (Without this the deeper chain is only reachable from a leaf via the ancestor
  // walk.) Other people's sub-replies stay one level down, X-style.
  const orderedReplies = deep
    ? visibleReplies
    : feedThreadReplies(clean, postAuthor, visibleReplies)
  const decorated = await decoratePosts(
    supabase,
    [postRow, ...visibleAncestors, ...orderedReplies],
    opts,
  )
