...
// While the feed is small, serve "For You" as a pure reverse-chronological
// timeline (newest first) rather than the engagement ranker + per-viewer follow
// nudge: with only a handful of posts a day, plain recency reads far more
// predictably than a reordered feed, and nothing gets buried. Flip this back to
// `true` to re-enable rankFeedCandidates + applyFollowNudge — all that machinery
// is kept intact below and gated only on this one flag, so it's a one-line
// return to ranking once volume justifies it.
const RANK_FORYOU_FEED = false
