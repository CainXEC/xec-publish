...
async function decoratePosts(supabase, rows, viewerAddress = '') {
  ...
  return attachQuoted(supabase, withEngagement)
}
 ...
 */
export async function getFeedPage({ page = 1, pageSize = 25, viewerAddress = '' } = {}) {
  ...
  const rowsList = rows ?? []
  const posts = await decoratePosts(supabase, rowsList, viewerAddress)
...
export async function getFollowingFeedPage({ page = 1, pageSize = 25, viewerAddress = '' } = {}) {
  ...
  const rowsList = rows ?? []
  const posts = await decoratePosts(supabase, rowsList, address)
  return { posts, hasNextPage: rowsList.length === pageSizeNum }
}
 ...
 */
export async function getFeedThread(txid, { viewerAddress = '' } = {}) {
  ...
  const [post] = await decoratePosts(supabase, [postRow], viewerAddress)
  const ancestors = await decoratePosts(supabase, ancestorRows, viewerAddress)
  const replies = await decoratePosts(supabase, replyRows ?? [], viewerAddress)
