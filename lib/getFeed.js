...
export async function getFeedThread(txid, { viewerAddress = '', viewerAccountId = null } = {}) {
  ...
  // Ancestor context: the parent chain above the focus post, root-first. A
  // recursive CTE (sql/feed_thread_ancestors.sql) walks it in ONE round-trip —
  // the walk is a linked list (each parent's txid is only known after loading
  // the previous row), so it used to be up to MAX_ANCESTORS serial queries. The
  // RPC already returns root-first and enforces the depth cap + cycle break, so
  // there's nothing to reverse or bound here.
  const { data: ancestorData, error: ancestorError } = await supabase.rpc(
    'get_feed_ancestors',
    { p_txid: clean, p_max_depth: MAX_ANCESTORS },
  )
  if (ancestorError) throw new Error(ancestorError.message)
  const ancestorRows = ancestorData ?? []
