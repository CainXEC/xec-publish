...
export async function getCachedForYouPage(cursor = null, pageSize = 25, viewerAccountId = null) {
  ...
  if (resolvedViewerAccountId) {
    // Two per-viewer reads on top of the shared window — block set + follow set —
    // fetched together. Blocks REMOVE posts; follows only REORDER, so filter
    // first, then nudge the survivors. nextCursor is time-based (finalizeWindow),
    // so reordering the display never affects pagination.
    ...
    posts = applyFollowNudge(posts, followed)
  }
