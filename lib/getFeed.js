...
export async function getFeedPage({ cursor = null, pageSize = 25, viewerAddress = '', viewerAccountId = null, attachIdentity = true } = {}) {
  ...
  return finalizeWindow(supabase, rows, pageSizeNum, {
    ...
    withTopReply: true,
    // Reverse-chronological while the feed is small (see RANK_FORYOU_FEED). The
    // rows are already fetched created_at DESC, id DESC, so rank=false serves
    // them newest-first as fetched.
    rank: RANK_FORYOU_FEED,
    // No spanMints: mint announcements are no longer woven into the feed.
  })
}
 ...
 */
/**
 * Fetch the account's repost events and hydrate the original posts so they can
 * be merged into the profile timeline. Returns entries shaped like feed_posts
 * rows but with two extra fields the merge step uses:
 *   _surfaceTs — the repost's created_at (when it should rank in the stream)
 *   _surfaceId — the repost event's id (tiebreaker, stays within feed_events)
 *   repostedBy — { accountId, identity } for the "Reposted by @X" byline
 */
async function fetchAccountRepostEntries(supabase, accountId, decoded, fetchSize) {
  let repostsQuery = supabase
    .from('feed_events')
    .select('id, target_txid, created_at, actor_identity')
  ...
  return events.flatMap((e) => {
    ...
    if (!orig) return []
    return [{ ...orig, _surfaceTs: e.created_at, _surfaceId: e.id, repostedBy: { accountId, identity: e.actor_identity } }]
  })
}
...
export async function getAccountFeedPage({ accountId, cursor = null, pageSize = 25, viewerAddress = '', viewerAccountId = null } = {}) {
  ...
  const [{ data: rows, error }, repostEntries] = await Promise.all([
    postsQuery,
    fetchAccountRepostEntries(supabase, account, decoded, FETCH_SIZE),
  ])
  if (error) throw new Error(error.message)
...
async function accountFeedPageNeutral(supabase, accountId, pageSizeNum) {
  ...
  const [{ data: rows, error }, repostEntries] = await Promise.all([
    ...
    fetchAccountRepostEntries(supabase, accountId, null, FETCH_SIZE),
  ])
  if (error) throw new Error(error.message)
...
export async function getFeedThread(txid, { viewerAddress = '', viewerAccountId = null, deep = false } = {}) {
  ...
  const [postRes, blocked, replyRes, ancestorRes] = await Promise.all([
    ...
    // `deep` (forum threads) fetches the WHOLE descendant tree for a Reddit-style
    // nested comment view; the default is just the direct replies (X-style).
    deep
  ...
  // A reply BY THE POST'S OWN AUTHOR floats to the top of the replies, so a
  // self-reply reads as a thread continuation directly under the original
  // (X-style "author thread") instead of sitting wherever it lands in time.
  // Node's sort is stable, so chronological order is preserved WITHIN each group —
  // multiple self-replies stay in order and read top-down. (Forum threads re-nest
  // + re-sort in ForumComments, so this only shapes the flat feed-thread view.)
  const postAuthor = postRow.author_account_id ?? null
  const orderedReplies = postAuthor
    ? [...visibleReplies].sort(
        (a, b) =>
          (a.author_account_id === postAuthor ? 0 : 1) -
          (b.author_account_id === postAuthor ? 0 : 1),
      )
    : visibleReplies
