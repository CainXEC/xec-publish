...
// uses the query's EXACT count, so "+N others" stays right even past this cap.
...
/**
 * Summarize a page span's mints into ONE digest row: name up to MINT_NAMED_MAX
 * handles, and collapse the rest into an "others" count — "@X, @Y, @Z and N
 * others minted handles". `count` is the span's EXACT total; the named handles
 * are a subset of it, so `others = count - named.length` never double-counts a
 * named handle. Spans are disjoint across pages, so each mint lands in exactly
 * one digest. The digest's txid is a synthetic stable key (never a real txid).
 */
function buildMintEntries(mints, mintCount) {
  if (!mints || mints.length === 0) return { digest: null }
  ...
  const total = Math.max(mintCount, named.length)
  return {
    digest: {
      ...
      count: total,
      named,
      others: Math.max(0, total - named.length),
...
async function finalizeWindow(
  ...
  const posts = weaveMintRows(decorated, [digest], signals, nowMs)
