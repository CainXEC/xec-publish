...
export async function searchPosts({ query, page = 1, pageSize = PAGE_SIZE }) {
  ...
  let pageQuery = supabase
    .from('posts')
    .select(
      'id, title, slug, teaser, reading_time_minutes, price_xec, created_at, published_at, author_id, authors(username)',
    )
    .eq('published', true)
  ...
  return {
    posts: posts.map((p) => ({
      ...
      display_handle: handleByAuthorId[p.author_id]?.handle ?? null,
      handle_color: handleByAuthorId[p.author_id]?.color ?? null,
    })),
