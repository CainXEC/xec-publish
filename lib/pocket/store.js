...
export const POCKET_SOFT_CAP_XEC = 50000
...
const DEFAULT_STATE = Object.freeze({
  ...
  accountId: null,
  primaryAddress: null,
  /** every wallet linked to the account (primary + old, still-linked ones) —
   *  the restore flow validates a pasted signature against the whole set. */
  linkedAddresses: [],
  sessionVia: null,
  ...
  spendPulse: 0,
  /** bumps whenever the optimistic-spend overlay SETTLES to the real Chronik
   *  balance (the ws-nudge fast path, ~1-2s, or the 20s safety-timer fallback
   *  if the nudge never lands) — i.e. balanceSats moving by just the tx fee,
   *  not new money. Consumers that watch balanceSats generically (the wordmark
   *  takeover) use this to tell "our own spend settling late" apart from a
   *  genuinely new balance change, which a fixed debounce window can't do
   *  once the settle is delayed past it. */
  reconcileSettlePulse: 0,
})
...
let reconcileSettlePulse = 0   // increments when the overlay settles — see DEFAULT_STATE
...
function emitBalance() {
  emit({ balanceSats: displayedSats(), spendPulse, reconcileSettlePulse })
}
...
export function applyOptimisticSpend(amountSats) {
  ...
  pendingClearTimer = setTimeout(() => {
    ...
    pendingSpentSats = 0
    // Same settle as the fast ws-nudge path above, just late — mark it the
    // same way so a generic "balance changed" watcher (the wordmark) can
    // still tell it apart from a fresh, unrelated balance change.
    reconcileSettlePulse += 1
    emitBalance()
    if (state.address) void refreshBalanceInner(state.address)
  }, 20000)
...
async function resolveAccount() {
  ...
  emit({
    ...
    linkedAddresses: Array.isArray(me.linkedAddresses) ? me.linkedAddresses : [],
