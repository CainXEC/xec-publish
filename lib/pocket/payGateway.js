/**
 * The Pocket — pay gateway.
 *
 * The drop-in wrapper around lib/ecash/cashtabPay.ts that every pay flow
 * calls instead of the raw trio. THE RULE: the pocket pays anything it can
 * afford, signing locally and broadcasting instantly; anything it can't cover
 * — and every failure/ineligibility — takes the EXACT Cashtab path the app
 * has today, automatically. Only proof-of-keys flows (login, change-address)
 * and NFT deliveries (mint, claim) never touch the pocket. With
 * NEXT_PUBLIC_POCKET_ENABLED off, every function here is a literal
 * pass-through: same calls, same arguments, same behavior.
 *
 * Contract (mirrors cashtabPay so call sites keep their shape):
 *   const handle = beginPayment({ kind, amountXec })   // SYNC, in the click
 *   ...await /prepare...
 *   const r = await completePayment(handle, { bip21, cashtabUrl })
 *   // r: { ok:true, via:'pocket', txid }              — pocket paid; POST the
 *   //                                                   txid to confirm NOW
 *   //    { ok:true, via:'extension'|'tab', ... }      — Cashtab path (as today)
 *   //    { ok:false, via:'extension', reason:'denied' } — user said no (as today)
 *   //    { ok:false, via:'pocket', reason:'pocket_error', message }
 *   //                                                 — pocket failed; pending
 *   //                                                   UI + manual Cashtab
 *   //                                                   link still apply
 *   abortPayment(handle)                               // /prepare failed
 */

import {
  payWithCashtab,
  beginCashtabPayment,
  completeCashtabPayment,
  abortCashtabPayment,
  isCashtabExtensionAvailable,
} from '@/lib/ecash/cashtabPay'
import {
  POCKET_ENABLED,
  getPocketSnapshot,
  getPocketSpendContext,
  refreshPocketBalance,
  applyOptimisticSpend,
  undoOptimisticSpend,
} from '@/lib/pocket/store'
import { parseEcashBip21 } from '@/lib/pocket/bip21'

/** The sats a pocket spend moves to the recipient, read off the BIP21 — what the
 *  optimistic balance drops by (the tiny network fee is corrected at reconcile). */
function spendSatsOf(bip21) {
  const parsed = parseEcashBip21(bip21)
  const sats = parsed ? Number(parsed.totalSats) : 0
  return Number.isFinite(sats) && sats > 0 ? sats : 0
}

// One shared load of the signer chunk: beginPayment's warm-up and
// completePayment's spend must never race two dynamic imports of the same
// module (wasteful, and it trips vitest's mock registry in tests).
let _walletModPromise = null
function walletMod() {
  return (_walletModPromise ??= import('@/lib/pocket/wallet'))
}

/** Headroom for the network fee + change dust so eligibility never green-lights
 *  a spend the builder would reject at the margin (~2000 sats ≫ any 1-2 input
 *  P2PKH fee at 1 sat/byte). */
const FEE_MARGIN_SATS = 2000

/** Estimate of the network fee on a pocket feed-action tx (~1-2 inputs, split
 *  outputs + OP_RETURN at 1 sat/byte). Folded into the OPTIMISTIC drop so the
 *  balance lands where Chronik will reconcile it — a spend costs amount + fee —
 *  instead of ticking down a couple of XEC a beat later (the "delayed glitch").
 *  Kept at/under the real fee so the overlay never over-shoots (which would make
 *  the balance tick back UP on reconcile); a tiny under-estimate just settles
 *  down a fraction of an XEC, usually invisible in the whole-XEC display. */
const POCKET_FEE_ESTIMATE_SATS = 300

/** THE RULE: the pocket pays anything it can afford; Cashtab handles the rest
 *  automatically. The only exclusions are STRUCTURAL, not price policy:
 *  login/change-address (the payment is proof of the WALLET's keys — a pocket
 *  payment proves nothing and is server-rejected) and mint/claim (the NFT is
 *  delivered to the paying address — it must land in the wallet, never on a
 *  browser hot key). Those flows never route through this gateway, and any
 *  unknown kind falls through to Cashtab as a backstop. */
const POCKET_KINDS = new Set([
  'feed-post',
  'feed-reply',
  'feed-quote',
  'feed-poll',
  'like',
  'tip',
  'repost',
  'unlock',
  'comment',
  'publish',
  // Forum creation is a 100%-platform FEE, and ownership binds to the caller's
  // ACCOUNT (runner_account_id from the session), not the paying address — so a
  // pocket payment is safe, exactly like a post. (Not a mint/claim: no NFT is
  // delivered to the payer.)
  'forum-create',
])

/**
 * Can the pocket pay this, right now? Pure/synchronous — safe inside a click
 * handler before any await. No price ceiling: the pocket's funding cap is the
 * bound, and a payment it can't cover routes to Cashtab automatically.
 * @param {string} kind    one of POCKET_KINDS (anything else → Cashtab)
 * @param {number} amountXec  total price in XEC (client always knows it at click)
 */
export function spendEligibility(kind, amountXec) {
  if (!POCKET_ENABLED) return { eligible: false, reason: 'disabled' }
  const snap = getPocketSnapshot()
  if (snap.status !== 'ready' || !snap.registered) return { eligible: false, reason: 'no_pocket' }
  if (!POCKET_KINDS.has(kind)) return { eligible: false, reason: 'kind' }
  const amount = Number(amountXec)
  if (!Number.isFinite(amount) || amount <= 0) return { eligible: false, reason: 'amount' }
  if (snap.balanceSats == null || snap.balanceSats < amount * 100 + FEE_MARGIN_SATS) {
    return { eligible: false, reason: 'balance' }
  }
  return { eligible: true }
}

/**
 * Warm the pocket signer AHEAD of a spend — load the wallet chunk + refresh the
 * UTXO set — so a later completePayment is build + broadcast only. Safe to call
 * eagerly (e.g. when a composer opens): a no-op when there's no spendable pocket,
 * idempotent, and ensureSynced's freshness window means a burst of calls is cheap.
 *
 * This matters now that a post/quote builds its payment CLIENT-SIDE and skips the
 * /prepare round trip the click warm-up used to overlap: without this, the sync
 * lands ON the click. Calling it when the composer opens moves the sync under the
 * user's read/type time instead — the biggest reason a freshly-opened quote/reply
 * composer felt a touch slower than the always-warm main composer.
 */
export function prewarmPocketSpend() {
  const ctx = getPocketSpendContext()
  if (!ctx) return
  void walletMod()
    .then((m) => m.warmPocketWallet(ctx.skHex))
    .catch(() => {})
}

/**
 * Start a payment inside the click gesture. Pocket-eligible → opens NOTHING
 * (no placeholder tab, no extension popup); otherwise identical to
 * beginCashtabPayment().
 */
export function beginPayment({ kind, amountXec }) {
  if (spendEligibility(kind, amountXec).eligible) {
    // Warm the signer (chunk load + UTXO sync) so completePayment is build +
    // broadcast only. Composers also prewarm on open (prewarmPocketSpend), so by
    // the click the sync is usually already fresh; this covers the cold case.
    prewarmPocketSpend()
    return { mode: 'pocket' }
  }
  return { mode: 'cashtab', gesture: beginCashtabPayment() }
}

/**
 * Finish a payment started with beginPayment once the BIP21 is known.
 * Pocket mode signs + broadcasts locally; on failure it silently retries via
 * the extension when present (extension popups need no window.open gesture),
 * else reports pocket_error and leaves the call site's pending UI — with its
 * existing manual "open Cashtab" affordance — to carry the payment. It never
 * window.open()s after an await (popup blockers eat those).
 */
export async function completePayment(handle, { bip21, cashtabUrl }) {
  if (handle?.mode !== 'pocket') {
    return completeCashtabPayment(handle.gesture, { bip21, cashtabUrl })
  }

  const ctx = getPocketSpendContext()
  if (ctx) {
    const { pocketSpend } = await walletMod()
    // Drop the displayed balance the instant we broadcast — the chip animates the
    // roll now; Chronik reconciles the exact figure a second later. Rolled back if
    // the spend fails (it then falls back to a Cashtab payment the pocket didn't
    // fund).
    const spendSats = spendSatsOf(bip21)
    const optimisticSats = spendSats > 0 ? spendSats + POCKET_FEE_ESTIMATE_SATS : 0
    applyOptimisticSpend(optimisticSats)
    const spend = await pocketSpend({ skHex: ctx.skHex, bip21 })
    if (spend.ok) {
      refreshPocketBalance()
      return { ok: true, via: 'pocket', txid: spend.txid }
    }
    undoOptimisticSpend(optimisticSats)
    if (isCashtabExtensionAvailable()) {
      // Desktop with the extension: hand the SAME bip21 to its popup — the
      // user sees a normal Cashtab approval instead of an error.
      return payWithCashtab({ bip21, cashtabUrl })
    }
    return { ok: false, via: 'pocket', reason: 'pocket_error', message: spend.error }
  }

  // The pocket vanished between click and completion (other-tab forget,
  // logout race). Extension can still take over silently; the web-tab path
  // can't (no gesture left), so report and let the pending UI carry it.
  if (isCashtabExtensionAvailable()) {
    return payWithCashtab({ bip21, cashtabUrl })
  }
  return {
    ok: false,
    via: 'pocket',
    reason: 'pocket_error',
    message: 'Pocket unavailable — pay with Cashtab instead.',
  }
}

/**
 * Like completePayment, but for the POCKET it resolves the instant the tx is
 * SIGNED (real txid known) instead of after the broadcast round-trip — so an
 * optimistic composer can show the post immediately and let the network catch up.
 *
 * Pocket: apply the optimistic balance drop, build+sign locally (fresh set so the
 * id is final), and return `{ ok, txid, broadcast }` where `broadcast` settles on
 * the background broadcast's result. A build that can't run (couldn't sync/sign)
 * undoes the drop and falls back exactly like completePayment (extension, else
 * pocket_error) — no post is shown. A broadcast that later fails undoes the drop
 * and reports via `broadcast`; the optimistic post was never recorded, so it
 * self-heals on the next feed refresh.
 *
 * Non-pocket (Cashtab tab/extension) has no pre-broadcast id — it delegates to
 * completePayment and returns a resolved `broadcast` so callers share one shape.
 */
export async function completePaymentOptimistic(handle, { bip21, cashtabUrl }) {
  const ctx = handle?.mode === 'pocket' ? getPocketSpendContext() : null
  if (!ctx) {
    const r = await completePayment(handle, { bip21, cashtabUrl })
    return { ...r, broadcast: Promise.resolve({ ok: r.ok }) }
  }

  const spendSats = spendSatsOf(bip21)
  const optimisticSats = spendSats > 0 ? spendSats + POCKET_FEE_ESTIMATE_SATS : 0
  applyOptimisticSpend(optimisticSats)

  const { pocketSpendDeferred } = await walletMod()
  const { built, done } = pocketSpendDeferred({ skHex: ctx.skHex, bip21 })
  const b = await built
  if (!b.ok) {
    // Never broadcast — undo the drop and fall back just like completePayment.
    undoOptimisticSpend(optimisticSats)
    if (isCashtabExtensionAvailable()) {
      const r = await payWithCashtab({ bip21, cashtabUrl })
      return { ...r, broadcast: Promise.resolve({ ok: r.ok }) }
    }
    return {
      ok: false,
      via: 'pocket',
      reason: 'pocket_error',
      message: b.error,
      broadcast: Promise.resolve({ ok: false }),
    }
  }

  // Signed: the id is real. Broadcast finishes in the background; reconcile the
  // balance chip on its result (undo the drop only if it actually failed).
  const broadcast = done.then((d) => {
    if (d.ok) {
      refreshPocketBalance()
      return { ok: true }
    }
    undoOptimisticSpend(optimisticSats)
    return { ok: false, error: d.error }
  })
  return { ok: true, via: 'pocket', txid: b.txid, broadcast }
}

/** Abandon a payment whose /prepare failed. Pocket mode opened nothing. */
export function abortPayment(handle) {
  if (handle?.mode === 'cashtab') abortCashtabPayment(handle.gesture)
}

/**
 * One-shot variant for flows whose BIP21 is known AT the click (unlock,
 * publish). `amountXec` may be omitted — the price is then read off the
 * BIP21 itself, so callers don't need to duplicate fee knowledge.
 */
export async function payDirect({ kind, amountXec, bip21, cashtabUrl }) {
  let amount = amountXec
  if (amount == null) {
    const parsed = parseEcashBip21(bip21)
    amount = parsed ? Number(parsed.totalSats) / 100 : NaN
  }
  if (spendEligibility(kind, amount).eligible) {
    return completePayment({ mode: 'pocket' }, { bip21, cashtabUrl })
  }
  return payWithCashtab({ bip21, cashtabUrl })
}
