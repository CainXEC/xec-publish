...
/**
 * Like verifySignatureAgainstPrimary, but against every wallet linked to the
 * account — not just the current primary. An address change keeps the OLD
 * address linked (non-primary) for recovery everywhere else in this system;
 * this check has to honor that too, or a Pocket originally set up under a
 * since-replaced primary becomes impossible to restore without first
 * swapping primary back. Returns the matching address (as passed in), or
 * null if none match.
 * @param {string} sigBase64
 * @param {string[]} addresses  every address linked to the account
 */
export function verifySignatureAgainstAny(sigBase64, addresses) {
  for (const addr of addresses ?? []) {
    if (verifySignatureAgainstPrimary(sigBase64, addr)) return addr
  }
  return null
}
