 ...
 * (post/reply/quote/repost/like) plus the three site actions that used to ride
 * a bare, LOKAD-less UUID push: article publish, paywall unlock, and wallet-auth
 * login. They now all share the same POWR frame so Cashtab can parse & label them.
 *
 ...
 *                             absent for repost/like/unlock/auth.
 *   [<push 36> nonce]         ASCII bytes of the auth challenge UUID. Present only
 *                             for auth (OP_8).
 *
...
// Actions that carry the 36-byte ASCII auth challenge nonce.
const CARRIES_NONCE = new Set([FEED_ACTION.AUTH])
...
export function decodeFeedOpReturn(scriptHex) {
  ...
  // Action: bare OP_N opcode (OP_1..OP_8)
  if (i + 2 > hex.length) return null
