 ...
 *                               OP_7 = unlock,  OP_8 = auth,     OP_9 = mint
 *                             (OP_9 is labelled "handle" in the frozen Cashtab
 *                              spec — same opcode; it's the handle-NFT mint payment)
 ...
 *                             absent for repost/like/unlock/auth/mint.
 *   [<push 36> nonce]         ASCII bytes of a challenge/id UUID. Present for auth
 *                             (OP_8, login nonce) and mint (OP_9, pending_mints id).
 *
 ...
 *   mint    : LOKAD | v0 | OP_9 |                          | nonce (36B mint id)
 *
...
// mint (the pending_mints id that ties the payment to the reserved handle).
const CARRIES_NONCE = new Set([FEED_ACTION.AUTH, FEED_ACTION.MINT])
