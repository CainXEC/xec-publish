...
// The author's share — used to rebuild the total from the clean AUTHOR leg when
// the FEE recipient is the payer (a forum runner reacting/replying in their own
// forum), which pollutes the fee leg with the tx change.
const AUTHOR_FEE_FRACTION = 1 - PLATFORM_FEE_FRACTION
...
export function matchFeedTx(tx, expected) {
  ...
  const carriesHash =
    ...
    action === FEED_ACTION.FORUM // commits sha256(slug)
  if (carriesHash && commit.contentHash !== String(contentHash).toLowerCase()) return null

  // Actions that reference another tx must commit to the expected target. Post
  // and forum-creation reference nothing.
  const carriesTarget = action !== FEED_ACTION.POST && action !== FEED_ACTION.FORUM
  if (carriesTarget) {
    if (!parentTxid || commit.targetTxid !== String(parentTxid).toLowerCase()) return null
  }

  // Payment shape: post, quote, and forum-creation are 100% platform; reply/like/
  // repost split 94/6 to the referenced post's author and the platform. A
  // SELF-reply (you reply to your own post) is forced 100% platform too —
  // `expected.platformOnly` — so you can't rebate 94% back to yourself.
  const platformOnly =
    action === FEED_ACTION.POST ||
    action === FEED_ACTION.QUOTE ||
  ...
  //
  // The one wrinkle is a payment where the PAYER is also one of the payees: the
  // tx's change returns to that address, so summing the outputs there folds the
  // change into the amount (a 100 XEC reply could read as 9,996 XEC). Whichever
  // leg is NOT the payer is clean, so we reconstruct the total from it via the
  // fixed 94/6 split; when neither leg is the payer, both are clean and we sum.
  //   - payer == author  → author leg polluted → rebuild from the platform leg.
  //   - payer == platform → the "platform" leg is the FORUM RUNNER's fee (forum
  //     fee-redirect), and the runner is reacting in their OWN forum, so it's
  //     polluted → rebuild from the clean author leg.
  let sats
  if (platformOnly) {
    ...
    // Normally the platform is never the payer so this output is the real amount.
    // But with forum fee-redirect the "platform" leg can be the runner's own
    // address (a self-action in their forum) — then it folds in change and can't
    // be trusted, so fall back to the expected floor.
    sats = payerAddress === platformAddress ? needSats : platformSats
  } else {
    ...
    if (payoutAddress === platformAddress) {
      // Author IS the fee recipient (engaging with the RUNNER's own post in their
      // forum): both the 94% and 6% legs land on the SAME address, so a single
      // satsToAddress already totals the whole payment — summing the two legs
      // would double-count it. When the payer is ALSO that address (the runner
      // engaging their own post), the outputs fold in change, so fall back to the
      // floor; otherwise the combined output IS the amount.
      sats = payerAddress === payoutAddress ? Math.floor(Number(costXec) * 100) : authorSats
    } else if (payerAddress === platformAddress) {
      // The FEE recipient (forum runner) is the payer: the fee leg is polluted by
      // change, but the author leg is clean. Rebuild the total from it (94%).
      sats = Math.round(authorSats / AUTHOR_FEE_FRACTION)
    } else if (payerAddress === payoutAddress) {
