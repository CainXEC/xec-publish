...
export function matchFeedTx(tx, expected) {
  ...
  // `sats` records the amount PAID for the action, which is the intended
  // price/split — NOT the sum of outputs to the payee. When the payer and the
  // payout address are the same wallet (replying to / liking your OWN post), the
  // tx's change output returns to that address, and summing outputs would fold
  // the change into the recorded amount (e.g. a 100 XEC reply shown as 4,256 XEC).
  let sats
  if (platformOnly) {
