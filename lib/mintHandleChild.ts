...
import { SLP_TOKEN_TYPE_NFT1_CHILD, payment, toHexRev, sha256d, fromHex } from "ecash-lib";
...
function txidOf(tx: any): string {
  return typeof tx.txid === "function" ? tx.txid() : toHexRev(sha256d(tx.ser()));
}
...
export async function mintHandleChild(wallet: Wallet, params: MintHandleParams): Promise<MintHandleResult> {
  ...
  // child tokenId = genesis txid; genesis is the LAST tx (after any chained split)
  const childTokenId = txidOf(built.txs[built.txs.length - 1]);

  const resp: any = await built.broadcast();
  const broadcastTxids = Array.isArray(resp) ? resp : built.txs.map(txidOf);

  return { childTokenId, handle, url, broadcastTxids };
}
