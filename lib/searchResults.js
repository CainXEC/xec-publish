...
export function profileRouteFor(identifier) {
  // A raw-address identity may carry the "ecash:" prefix; the /@ route wants the
  // bare form (a handle never starts with "ecash:", so stripping is a no-op for
  // handles). Keeps address profile links canonical, never /@ecash:<addr>.
  const id = String(identifier ?? '').replace(/^ecash:/i, '')
