...
export async function getFeedNotifications(supabase, accountId, { limit = 20, before = null } = {}) {
  ...
  if (feedPostTxids.length > 0 || commentTxids.length > 0) {
    try {
      ...
      for (const n of rows) {
        if (n.type === 'reply' && n.action_txid) {
          const p = postByTxid.get(n.action_txid)
          n.actionContent = p?.deleted_at ? null : p?.content ?? null
        } else if (n.type === 'quote' && n.post_txid) {
          const p = postByTxid.get(n.post_txid)
          n.actionContent = p?.deleted_at ? null : p?.content ?? null
        } else if (n.type === 'comment' && n.action_txid) {
          const c = commentByTxid.get(n.action_txid)
          n.actionContent = c?.deleted_at ? null : c?.content ?? null
          // parent_id present -> this "comment" is actually a reply to another
          // comment, not a top-level comment on the article — a nicer verb.
          n.isCommentReply = Boolean(c?.parent_id)
        } else if ((n.type === 'like' || n.type === 'repost') && n.post_txid) {
          const p = postByTxid.get(n.post_txid)
          n.targetContent = p?.deleted_at ? null : p?.content ?? null
        }
      }
    } catch {
