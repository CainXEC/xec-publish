...
export async function getFeedNotifications(supabase, accountId, { limit = 20, before = null } = {}) {
  ...
  // MENTION is article-linked too (post_txid = the article's posts.id, and it
  // carries no action_txid — that's how it's told apart from a FEED mention,
  // whose post_txid is a 64-hex feed txid).
  const articleRows = rows.filter(
    (n) =>
      (ARTICLE_NOTIF_TYPES.includes(n.type) || (n.type === 'mention' && !n.action_txid)) &&
      n.post_txid,
  )
