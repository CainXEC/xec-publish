...
export async function getFeedNotifications(
  supabase,
  accountId,
  { limit = 20, before = null, types = null } = {},
) {
  ...
  let listQuery = supabase
    ...
    .limit(limit)
  // Optional type filter — powers the "Mentions" tab (reply/quote/mention/
  // comment) so it pages over ONLY respond-worthy rows, not the full mixed feed.
  if (Array.isArray(types) && types.length > 0) listQuery = listQuery.in('type', types)
