...
//  now record here too, via /api/follows. See sql/feed_notifications.sql.
//
...
export const ARTICLE_NOTIF_TYPES = ['unlock', 'comment']
...
export async function resolveAccountByAddress(supabase, address) {
  ...
  return {
    accountId,
    identity: acct?.display_handle ? `@${acct.display_handle}` : `ecash:${bare}`,
  }
}
...
export async function recordArticleNotification(
  supabase,
  { postId, type, actorAccountId = null, actorIdentity = null, actorAddress = null },
) {
...
export async function recordFeedNotification(
  ...
  try {
    const { error } = await supabase.from('feed_notifications').insert({
      ...
      type,
      post_txid: postTxid,
      // The tip amount in sats for a paid reaction (a like). Null for types with
      // no payment to the recipient (follow, etc.); the bell renders the plain
      // verb when it's absent.
