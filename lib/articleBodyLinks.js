...
import { powInternalHref, externalUrlHref, tokenizeContent, textMayContainLink } from './contentLinks'
...
function linkifyText(doc, root) {
  ...
  while (walker.nextNode()) {
    ...
    // shared with tokenizeContent's own detectors (see textMayContainLink), so
    // this can't silently miss what the real tokenizer would catch. A narrower,
    // hand-rolled version of this check used to require "https://", missing a
    // bare "proofofwriting.com/…" link entirely.
    if (textMayContainLink(node.nodeValue)) targets.push(node)
  }
