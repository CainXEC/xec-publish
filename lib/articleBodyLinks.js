...
// Pass 0: a YouTube URL on its OWN line (a paragraph that is JUST the URL) becomes
// an embedded player. Scoped to "sole content of a block" so we never rip a link
// out of the middle of a sentence; a YouTube URL inline in prose is left alone.
function embedYouTube(doc, root) {
  for (const p of [...root.querySelectorAll('p')]) {
    ...
    if (!id) continue
    if (stripYouTubeLink(text).trim() !== '') continue // more than just the URL
    p.replaceWith(makeYouTubeEmbed(doc, id))
  }
}
