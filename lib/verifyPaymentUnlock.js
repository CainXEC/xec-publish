...
export async function verifyAndRecordUnlock({
  ...
  try {
    ...
    console.log(`${logPrefix} getOutputScriptFromAddress(author) error`, {
      message: e?.message,
      authorXecAddress,
      err: e,
    })
  ...
  try {
    ...
    console.log(`${logPrefix} getOutputScriptFromAddress(platform) error`, {
      message: e?.message,
      platformXecAddress,
      err: e,
    })
  ...
  const shouldLogOutputMatching = verbose
  if (shouldLogOutputMatching) {
    ...
    const authorMatchedPreFind = tx.outputs.some(
      (output) =>
        output.outputScript === authorOutputScript &&
        output.sats >= requiredAuthorSats,
    )
    const platformMatchedPreFind = tx.outputs.some(
      (output) =>
        output.outputScript === platformOutputScript &&
        output.sats >= requiredPlatformSats,
    )
    console.log(`${logPrefix} aggregate match (before find)`, {
      authorMatched: authorMatchedPreFind,
      platformMatched: platformMatchedPreFind,
    })
  }
