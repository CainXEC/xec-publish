...
export async function verifyAndRecordUnlock({
  ...
  // Outputs check out, but don't grant access until the payment is Avalanche-
  // final. A 0-conf tx can still be replaced by a conflicting double-spend, so
  // recording the unlock now would hand out a "free article" that the payer can
  // then claw back. `reason: 'finalizing'` tells the route to answer 202 so the
  // client keeps polling until finality lands (~2-3s), rather than erroring.
  if (!txIsFinal(tx)) {
    return { ok: false, reason: 'finalizing', error: 'Payment is finalizing' }
  }
  ...
  // Tell the author someone bought their article. Best-effort: never blocks or
  // fails the unlock (the reader already paid and is entitled). Skips silently
  // when the payer has no account or is the author themselves.
  await recordArticleNotification(supabase, {
    postId,
    type: 'unlock',
    actorAddress: payerAddress,
  })
