...
export async function verifyAndRecordUnlock({
  ...
  if (verbose) {
    console.log(`${logPrefix} amount-only verification`, {
      requiredAuthorSats: requiredAuthorSats.toString(),
      requiredPlatformSats: requiredPlatformSats.toString(),
      authorXecAddress,
      platformXecAddress,
    })
  }
  ...
  const authorOutputIndex = tx.outputs.findIndex(
    (output) => output.sats >= requiredAuthorSats,
  )
  const authorOutput =
    authorOutputIndex >= 0 ? tx.outputs[authorOutputIndex] : null
  const platformOutput = tx.outputs.find(
    (output, idx) => idx !== authorOutputIndex && output.sats >= requiredPlatformSats,
  )
