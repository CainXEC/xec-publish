...
// Unlock price bounds, in whole XEC. Mirrors NewPostForm's client-side check.
export const ARTICLE_MIN_PRICE_XEC = 100
export const ARTICLE_MAX_PRICE_XEC = 1_000_000
 ...
 * }} input
 * @returns {Promise<{ ok: true, id: string | null, finalSlug: string, storedBody: string, publishedChanged: boolean }
...
export async function savePostCore(admin, authorId, input = {}) {
  ...
  // Server-enforced unlock price bounds — the REAL gate. NewPostForm validates
  // too (100 min / 1,000,000 max), but the client can be bypassed (a crafted
  // request, or a draft autosave that skips the publish check), so clamp here to
  // whole XEC within [100, 1,000,000]. Non-numeric input falls back to the floor.
  const priceNum = Number(input.priceXec)
  const safePrice = Number.isFinite(priceNum)
    ? Math.min(ARTICLE_MAX_PRICE_XEC, Math.max(ARTICLE_MIN_PRICE_XEC, Math.floor(priceNum)))
    : ARTICLE_MIN_PRICE_XEC
  ...
  if (targetId) {
    ...
    // Whether THIS write touched the published column at all (either
    // direction) — the caller uses it to decide whether the front page rail's
    // cached list needs busting. An autosave that doesn't carry `published`
    // (see the comment above) can't have changed what the rail shows.
    return {
      ok: true,
      id: updatedRow.id,
      finalSlug,
      storedBody: bodyTrimmed,
      publishedChanged: 'published' in payload,
    }
  }
  ...
  return {
    ...
    finalSlug,
    storedBody: bodyTrimmed,
    // A fresh insert is always published:false (never yet shown on the rail),
    // so there's nothing there for the caller to bust.
