...
/**
 * Lean data for the write-page rails: the author's recent posts (drafts +
 * published, for the quick-switch list) and a stats glance (unlock count, summed
 * earnings, follower count). A few indexed queries — cheap enough to run inline
 * on the server page without Suspense. Never throws; degrades to empties so the
...
export async function getWriteSidebarData({ authorId = null, accountId = null } = {}) {
  const empty = { drafts: [], stats: { unlocks: 0, earnedXec: 0, followers: 0 } }
  const admin = createSupabaseAdminClient()
  if (!admin) return empty

  const [postsQ, unlocksQ, followersQ] = await Promise.all([
    // Recent posts (drafts + published) for the quick-switch list.
    authorId
      ? admin
          .from('posts')
