...
//      costs real fees. So distinct paying supporters is the biggest lever. (A
//      raw-XEC-amount term used to ride alongside as a hard-saturated anti-whale
//      boost; it was removed 2026-08-27 once posts lost direct tips and reactions
//      went flat at 100 XEC — with no amount variance left, it discriminated
//      nothing breadth didn't already capture.)
 ...
 * @param {{distinctSupporters:number}} [signal]  The already-clean paid-engagement
 *   signal for this post (self/linked/AI reactions already excluded by the RPC).
 *   Absent → breadth contributes nothing and the score degrades gracefully to
 *   recency + conversation.
 * @param {number} nowMs
 */
function scorePost(post, signal, nowMs) {
  ...
  return breadthBoost + convoBoost + exploreBoost - ageHours
}
 ...
 * @param {Map<string, {distinctSupporters:number}>} [signals]
