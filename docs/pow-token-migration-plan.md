...
Spread the founder's ~582K held POW to **real, paying participants**, weekly,
pro-rata to the **platform revenue each account generated** — ungameable by
construction.

### The metric — platform-*received* XEC per account per ISO week
Score each account by the XEC that platform-controlled addresses actually received
from that account's activity in the week. Author payouts are **excluded** (the 94%
can be shuffled between colluding accounts); only the platform's cut counts. Two
on-chain revenue sinks are scanned:
- **Platform fee address** (`ecash:qrw35…`, `PLATFORM_XEC_ADDRESS`) — the 6% cut on
  unlocks / posts / paid comments / reactions, plus 100%-to-platform actions (e.g.
  forum-create fees).
- **Mint wallet** (`ecash:qpw8gv2d…`, `MINT_PAYMENT_ADDRESS`) — NFT mint revenue.
- **EXCLUDING 👎 downvotes** — they pay the platform 100%, so counting them would
  literally *pay people to trash posts*. Dropped by txid (`feed_events.emoji='👎'`).

**How it's computed** (see [lib/powRewards/tallyWeek.ts](../lib/powRewards/tallyWeek.ts)):
scan both addresses' on-chain receipts for the week, attribute each to the paying
account by **sender (first input) → `account_addresses`** (so fees paid from the
Pocket count — it's a proven address), and sum. **Token-bearing outputs are
...
### Allocation
```
yourPOW = weeklyPool × (yourPlatformFeeSats_thisWeek / totalPlatformFeeSats_thisWeek)
```
Fixed weekly pool, pure pro-rata.
...
- **Only platform-received XEC counts**, so collusion (tipping/unlocking each other)
  earns nothing — you'd have to send real money to the *platform*.
- The pool is **fixed and diluted pro-rata**, and POW has **no market price**, so
  farming is **negative-sum** (pay real XEC to earn a token worth ≤ what you paid,
  split thinner the harder you push).
- **`account_links` cluster filter** collapses an alt-cluster to one participant
  (the existing "filter before the tally" infra).
- **`is_ai` house accounts excluded.**
